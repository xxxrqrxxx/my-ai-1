...
const FileCard = ({ file, isUser, onClick }) => {
  ...
  return (
    <div onClick={onClick} style={{
      ...
      padding: '10px 12px 10px 10px', borderRadius: 18,
      ...
      maxWidth: 260, minWidth: 170, cursor: 'pointer',
    }}>
      <div style={{
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ fontSize: 10, color: 'var(--text-muted)', marginTop: 2 }}>点击预览</div>
      </div>
      <span style={{
        fontSize: 9, background: isUser ? 'var(--accent)' : 'var(--accent-lighter)',
        color: isUser ? 'white' : 'var(--accent)', padding: '2px 7px', borderRadius: 6,
...
const ImageCard = ({ src, onClick }) => {
  return (
    <div onClick={onClick} style={{
      width: 180, height: 180, borderRadius: 18, overflow: 'hidden',
      cursor: 'pointer', border: '2px solid var(--accent-lighter)',
      ...
      <div onClick={(e) => e.stopPropagation()} style={{
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, flex: 1, minWidth: 0 }}>
            <div style={{
              width: 32, height: 32, borderRadius: 10, background: 'var(--accent-gradient)',
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            <button onClick={handleDownload} style={{
              width: 34, height: 34, borderRadius: 17, background: 'var(--glass-bg)',
        ...
        <div style={{ flex: 1, overflow: 'auto', background: 'white' }}>
          ...
          {isPdf && !file.dataUrl && (
            <div style={{ padding: 40, textAlign: 'center', color: 'var(--text-muted)', fontSize: 14 }}>
              PDF 已发送给 Arden<br />刷新后预览需重新上传文件
            </div>
          )}
          {isText && file.content && (
            <pre style={{ margin: 0, padding: 16, fontSize: 13, lineHeight: 1.6, color: '#333', whiteSpace: 'pre-wrap', wordBreak: 'break-word', fontFamily: 'monospace' }}>
              {file.content}
            </pre>
          )}
          {!isHtml && !isPdf && !isText && (
            <div style={{ padding: 40, textAlign: 'center', color: 'var(--text-muted)', fontSize: 14 }}>该文件类型暂不支持预览</div>
          )}
        </div>
      </div>
    </div>
...
const HtmlPreview = ({ html }) => {
  ...
  const handleOpenNew = () => { const blob = new Blob([html], { type: 'text/html' }); const url = URL.createObjectURL(blob); window.open(url, '_blank'); };
  return (
    <div style={{ marginTop: 8, maxWidth: '88%', alignSelf: 'flex-start', borderRadius: 18, overflow: 'hidden', background: 'var(--glass-bg)', backdropFilter: 'blur(20px)', border: '1px solid var(--glass-border)', boxShadow: 'var(--shadow)' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px', background: 'linear-gradient(135deg, rgba(245,202,216,0.45), rgba(240,188,204,0.3))', borderBottom: '1px solid var(--glass-border)' }}>
        ...
        <div style={{ display: 'flex', gap: 6 }}>
          ...
          <button onClick={handleOpenNew} style={{ background: 'var(--glass-bg-strong)', border: 'none', borderRadius: 8, padding: '4px 10px', fontSize: 11, color: 'var(--text-secondary)', cursor: 'pointer' }}>新窗口</button>
        </div>
      </div>
      {expanded ? (
        ...
        <div onClick={() => setExpanded(true)} style={{ padding: '28px 20px', textAlign: 'center', cursor: 'pointer' }}>
          <div style={{ width: 48, height: 48, borderRadius: 24, background: 'var(--accent-gradient)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 10px', boxShadow: 'var(--shadow-accent)' }}>
...
const extractHtmlBlocks = (content) => {
  ...
  const blocks = [];
  let match;
  while ((match = htmlRegex.exec(content)) !== null) { blocks.push(match[1].trim()); }
...
export default function ChatView({ aiName, userName, onOpenSidebar, sessionId, settings }) {
  ...
  const getTime = () => { const now = new Date(); return `${String(now.getHours()).padStart(2, '0')}:${String(now.getMinutes()).padStart(2, '0')}`; };

  const filteredMessages = searchQuery.trim() ? messages.filter(m => m.content?.includes(searchQuery.trim())) : messages;
  ...
  const handleSend = async () => {
    ...
    if (selectedFile?.content) {
      ...
      sendContent = `[用户上传了文件：${selectedFile.name}（二进制文件，无法直接读取内容）] ${sendContent}`.trim();
    }
  ...
  const confirmPoke = () => { if (pokeAction && pokePart) setShowPokePanel(false); };

  const handleImageSelect = (e) => {
    const file = e.target.files?.[0];
    if (!file) return;
    const reader = new FileReader();
    reader.onload = (ev) => { setSelectedImage({ name: file.name, dataUrl: ev.target.result }); };
  ...
  const handleFileSelect = (e) => {
    ...
    if (isText) {
      const reader = new FileReader();
      reader.onload = (ev) => { setSelectedFile({ name: file.name, content: ev.target.result, type: 'text' }); };
      reader.readAsText(file);
    } else if (isPdf) {
      const reader = new FileReader();
      reader.onload = (ev) => { setSelectedFile({ name: file.name, content: null, dataUrl: ev.target.result, type: 'pdf' }); };
      reader.readAsDataURL(file);
    } else {
      setSelectedFile({ name: file.name, content: null, dataUrl: null, type: 'binary' });
    }
    setShowPlusMenu(false);
  };
    ...
    <div className="page-container" onClick={() => setShowPlusMenu(false)}>
      <div className="glass-header" style={{ position: 'relative', zIndex: 50, flexShrink: 0, paddingTop: 'env(safe-area-inset-top)' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px' }}>
          <button onClick={onOpenSidebar} className="jelly-button" style={{ width: 40, height: 40 }}><Icon name="menu" /></button>
          <h2 style={{ fontSize: 18, fontWeight: 600, color: 'var(--text-primary)', flex: 1, textAlign: 'center' }}>{aiName}</h2>
          <div style={{ display: 'flex', gap: 6 }}>
            <button onClick={(e) => { e.stopPropagation(); setShowSearch(!showSearch); }} className="jelly-button" style={{ width: 38, height: 38 }}><Icon name="search" size={18} /></button>
            <button onClick={(e) => { e.stopPropagation(); setShowModelPicker(true); }} className="jelly-button" style={{ width: 38, height: 38 }}><Icon name="api" size={18} /></button>
          </div>
        </div>
        {showSearch && (
          <div style={{ padding: '0 16px 12px' }} onClick={(e) => e.stopPropagation()}>
            <div className="jelly-card" style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 14px' }}>
              ...
              <input type="text" placeholder="搜索聊天记录..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} style={{ flex: 1, border: 'none', background: 'transparent', fontSize: 14, color: 'var(--text-primary)', outline: 'none' }} autoFocus />
              {searchQuery && <button onClick={() => setSearchQuery('')} style={{ background: 'none', border: 'none', padding: 2 }}><Icon name="x" size={14} /></button>}
            </div>
          </div>
        )}
      </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '16px 16px 0' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12, paddingBottom: 12 }}>
          {loadingHistory && <div style={{ textAlign: 'center', color: 'var(--text-muted)', fontSize: 12, padding: 20 }}>加载消息中...</div>}
          {!loadingHistory && messages.length === 0 && <div style={{ textAlign: 'center', color: 'var(--text-muted)', fontSize: 13, padding: 40 }}>和 {aiName} 说点什么吧～</div>}
          {filteredMessages.map(msg => {
            if (msg.role === 'system') return (
              <div key={msg.id} style={{ textAlign: 'center', margin: '8px 0' }}><span className="poke-system-msg">{msg.content}</span></div>
            );
            return (
              <div key={msg.id} style={{ display: 'flex', flexDirection: 'column', alignItems: msg.role === 'user' ? 'flex-end' : 'flex-start', width: '100%' }}>
                ...
                {msg.role === 'assistant' ? (() => {
                  ...
                  return (<>
                    {text && <div className="bubble-ai" style={{ padding: '12px 16px', fontSize: 15, lineHeight: 1.6, maxWidth: '80%', whiteSpace: 'pre-wrap', wordBreak: 'break-word', borderRadius: 20 }}>{text}</div>}
          ...
          {isTyping && (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
              <div className="bubble-ai" style={{ padding: '14px 18px', borderRadius: 20 }}><span className="thinking-dots"><span /><span /><span /></span></div>
            </div>
          )}
          ...
          <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 6, gap: 8 }}>
            {selectedImage && (
              <div className="jelly-card" style={{ padding: '6px 8px 6px 6px', display: 'flex', alignItems: 'center', gap: 6, fontSize: 12 }}>
                ...
                <button onClick={() => setSelectedImage(null)} style={{ background: 'none', border: 'none', padding: 2 }}><Icon name="x" size={12} /></button>
              </div>
            )}
            {selectedFile && (
              <div className="jelly-card" style={{ padding: '6px 12px', display: 'flex', alignItems: 'center', gap: 6, fontSize: 12 }}>
                ...
                <button onClick={() => setSelectedFile(null)} style={{ background: 'none', border: 'none', padding: 2 }}><Icon name="x" size={12} /></button>
              </div>
            )}
          </div>
        )}
        {pokeAction && pokePart && (
          <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 6 }}>
            <div className="poke-system-msg" style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 14px', fontSize: 13 }}>
              ...
              <button onClick={() => { setPokeAction(null); setPokePart(null); }} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 2 }}><Icon name="x" size={14} /></button>
            </div>
          </div>
        )}
        <div style={{ position: 'relative' }}>
          {showPlusMenu && (
            <div className="plus-menu" onClick={(e) => e.stopPropagation()}>
              <button onClick={() => imageInputRef.current?.click()} className="plus-menu-item"><Icon name="image" size={18} color="var(--accent)" /><span>添加图片</span></button>
              <button onClick={() => fileInputRef.current?.click()} className="plus-menu-item"><Icon name="file" size={18} color="var(--accent)" /><span>添加文件</span></button>
              <button onClick={() => { setShowPokePanel(true); setShowPlusMenu(false); }} className="plus-menu-item"><Icon name="finger" size={18} color="var(--accent)" /><span>戳一戳</span></button>
            </div>
          )}
          <div className="jelly-card" style={{ display: 'flex', alignItems: 'flex-end', gap: 6, padding: 6 }} onClick={(e) => e.stopPropagation()}>
            <div style={{ position: 'relative' }}>
              <button onClick={() => setShowPlusMenu(!showPlusMenu)} className="jelly-button" style={{ width: 38, height: 38 }}><Icon name="plus" /></button>
            ...
            <textarea value={inputText} onChange={(e) => setInputText(e.target.value)} onKeyDown={handleKeyDown} placeholder="说点什么..." rows={1} style={{ flex: 1, border: 'none', background: 'transparent', padding: '10px 4px', fontSize: 15, color: 'var(--text-primary)', outline: 'none', resize: 'none', maxHeight: 100, fontFamily: 'inherit' }} />
            <button onClick={handleSend} className="jelly-button jelly-button-accent" style={{ width: 40, height: 40 }}><Icon name="send" size={18} /></button>
          </div>
        </div>
      </div>
